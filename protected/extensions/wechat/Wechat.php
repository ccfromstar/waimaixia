...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
	public function getJsTicket($appid='',$jsapi_ticket='',$type='jsapi'){
		...
		$result = $this->http_get(self::API_URL_PREFIX.self::GET_TICKET_URL.'access_token='.$this->access_token.'&type='.$type);
		if ($result)
		{
			$json = json_decode($result,true);
