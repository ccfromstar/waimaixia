...
class EuploadifyWidget extends CInputWidget {
    ...
    protected $validOptions = array(
        ...
        'progressData' => array('type' => 'string', 'possibleValues' => array('percentage', 'speed', 'all')),
        ...
        'createFolder' => array('type' => 'boolean'),
        'checkExisting' => array('type' => 'string'), //the php file use to check the upload file is exist
        ...
        'transparent' => array('type' => 'boolean'), // Set to transparent to make the background of the flash file transparent.     
    ...
    public function run() {
        ...
        $clientScript->registerScriptFile($this->baseUrl . '/uploadify/jquery.uploadify.js', CClientScript::POS_HEAD);
        $clientScript->registerScriptFile($this->baseUrl . '/uploadify/flash_detect.1.0.4.min.js', CClientScript::POS_HEAD);
        //$clientScript->registerScriptFile($this->baseUrl . '/uploadify/uploadifyLang_en.js', CClientScript::POS_HEAD);
...
EOP;
        ...
        if (!empty($validOptions)) {
            foreach ($value as $key => $val) {
                ...
                if ((!is_array($validOptions[$key]['type']) && ($type != $validOptions[$key]['type'])) || (is_array($validOptions[$key]['type']) && !in_array($type, $validOptions[$key]['type']))) {
                    throw new CException(Yii::t('EUploadify', '{k} must be of type {t}', array('{k}' => $key, '{t}' => $validOptions[$key]['type'])));
                }
    ...
    protected function makeOptions() {
        if (!array_key_exists('cancelImg', $this->options))
            $this->options['cancelImage'] = $this->baseUrl . '/uploadify/uploadify-cancel.png';
        if (!array_key_exists('swf', $this->options))
            $this->options['swf'] = $this->baseUrl . '/uploadify/uploadify.swf';
        if (!array_key_exists('langFile', $this->options))
            $this->options['langFile'] = $this->baseUrl . '/uploadify/uploadifyLang_en.js';
        if (!array_key_exists('buttonText', $this->options))
            $this->options['buttonText'] = '选择文件';
        if(!array_key_exists('queueSizeLimit',$this->options))
            $this->options['queueSizeLimit'] = 8;
