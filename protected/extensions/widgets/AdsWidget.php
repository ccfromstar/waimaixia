...
class AdsWidget extends CWidget
{
    ...
    public function run()
    {
        ...
        $criteria->condition = 'posid=:posid and status=1';
        $today = date('Y-m-d');
        $criteria->addCondition('stime<=:st and etime>=:et');
        $criteria->params = array(
            ':posid' => $this->model->id,
            ':st' => $today,
            ':et' => $today,
        );
        $criteria->limit = $this->model->nums;
        ...
        switch ($this->model->type) {
            ...
            case 1: //幻灯
                $list = '';
                foreach($ads as $ad) {
                    $img = CHtml::tag('img',array('style' => 'border:none;','width'=>$this->model->width,'height'=>$this->model->height, 'src' => $ad->pic, 'alt' => $ad->title,'text'=>$ad->description));
                    $link = CHtml::tag('a', array('href' => Yii::app()->controller->createUrl('ads/show', array('id' => $ad->id)), 'target' => '_blank'), $img);
                    $list .= CHtml::tag('li',array(),$link);
                }
                $ul = CHtml::tag('ul', array(), $list);
