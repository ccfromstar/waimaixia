	...
	*/
	public function actionUpdate()
	{
		...
		if ($model->type == 2) {
			...
			$db->text = 'select lx from AuthItem where type=:type and name=:name';
			$db->params = array(':type' => $model->type, ':name' => $model->name);
			$model->lx = $db->queryScalar();
		}
		// Create the authorization item form
		...
		if( isset($_POST['AuthItemForm'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
				...
				$this->_authorizer->updateAuthItem($itemName, $formModel->name, $formModel->description, $formModel->bizRule, $formModel->data, $formModel->lx);
